...
# raiders
raiders_roles = read_database("tarkov_bot", "raiders", "raiders_roles")
...
async def on_ready():
	...
	role_assignment_message_id = read_database("tarkov_bot", "role_assignment", "role_assignment_message_id")
	raid_notification_channel = client.get_channel(
		role_assignment_channel_id)  # channel (id) in which the bot announces a raid

	# CODE
	if role_assignment_message_id is not None:
		await client.http.delete_message(raid_notification_channel.id, role_assignment_message_id)
		...
		if str(reaction.message.author) == bot_client:
			if str(user) != bot_client:
				...
				for role in user.roles:
					if str(role) == role_assignment_role:
						has_not_role = False
				if not has_not_role:
...
async def on_reaction_remove(reaction, user):
	...
	# Role assignment message
	elif str(reaction.message.id) == str(role_assignment_message_id):
		has_role = None
		for role in user.roles:
			if str(role) == role_assignment_role:
				has_role = True
		if has_role:
			...
			await user.send(f"The removal of the {role_assignment_role} role was successful.")

...
class ReactionPayload:
    ...
    async def _init(self, payload):
        self.guild: Guild = config.bot.get_guild(payload.guild_id)
        self.member: Member = await self.guild.fetch_member(payload.user_id)
        self.emoji = payload.emoji
        self.channel: TextChannel = config.bot.get_channel(payload.channel_id)
        self.message: Message = await self.channel.fetch_message(payload.message_id)
